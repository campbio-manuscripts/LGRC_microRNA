...
summary(lm(fev1 ~ reads + protocol + smoke + gender + age + sample.cluster.f))
...
anova(lm(fev1 ~ reads + protocol + smoke + gender + age + sample.cluster.f))
...
summary(lm(dlco ~ reads + protocol + smoke + gender + age + sample.cluster.f))
...
anova(lm(dlco ~ reads + protocol + smoke + gender + age + sample.cluster.f))
...
summary(lm(bode ~ reads + protocol + smoke + gender + age + sample.cluster.f))
...
anova(lm(bode ~ reads + protocol + smoke + gender + age + sample.cluster.f))
...
summary(lm(fev1.fvc ~ reads + protocol + smoke + gender + age + sample.cluster.f))
...
anova(lm(fev1.fvc ~ reads + protocol + smoke + gender + age + sample.cluster.f))
...
summary(lm(emphysema ~ reads + protocol + smoke + gender + age + sample.cluster.f))
...
anova(lm(emphysema ~ reads + protocol + smoke + gender + age + sample.cluster.f))
...
boxplot(dlco ~ sample.cluster.f, group.names=1:5, ylab="DLCO", col="grey95", las=1)
boxplot(fev1 ~ sample.cluster.f, group.names=1:5, ylab="FEV1 Percent Predicted", col="grey95", las=1)
boxplot(emphysema ~ sample.cluster.f, group.names=1:5, ylab="Percent Emphysema", col="grey95", las=1)
boxplot(bode ~ sample.cluster.f, group.names=1:5, ylab="BODE", col="grey95", las=1)
boxplot(dlco[ild.ind] ~ sample.cluster.f[ild.ind], group.names=1:5, ylab="DLCO", col="purple", las=1)
boxplot(dlco[copd.ind] ~ sample.cluster.f[copd.ind], group.names=1:5, ylab="DLCO", col="yellow", las=1)
boxplot(fev1[copd.ind] ~ sample.cluster.f[copd.ind], group.names=1:5, ylab="FEV1 Percent Predicted", col="yellow", las=1)
boxplot(fev1.fvc[copd.ind] ~ sample.cluster.f[copd.ind], group.names=1:5, ylab="FEV1/FVC", col="yellow", las=1)
boxplot(emphysema[copd.ind] ~ sample.cluster.f[copd.ind], group.names=1:5, ylab="Percent Emphysema", col="yellow", las=1)
boxplot(bode[copd.ind] ~ sample.cluster.f[copd.ind], group.names=1:5, ylab="BODE", col="yellow", las=1)

plot(1:10, type="n", xaxt='n', yaxt='n', ann=FALSE, axes=FALSE, bty="n")
legend("center", legend = c("All samples", "ILD only", "COPD only"), fill = c("grey95", "purple", "yellow"))

dev.off()


pdf("Sample_Cluster_Assocations_By_Disease.pdf", useDingbats=FALSE, width=8.5, height=11)
par(mfrow=c(6,1), mar=c(2,4,2,2))
boxplot(dlco ~ status + sample.cluster.f, names=rep(1:5, each=3), ylab="DLCO", col=c("grey50", "purple", "yellow"), las=1)
boxplot(fev1 ~ status + sample.cluster.f, names=rep(1:5, each=3), ylab="FEV1 Percent Predicted", col=c("grey50", "purple", "yellow"), las=1)
boxplot(emphysema ~ status + sample.cluster.f, names=rep(1:5, each=3), ylab="Percent Emphysema", col=c("grey50", "purple", "yellow"), las=1, xlab="Sample cluster")
boxplot(bode ~ status + sample.cluster.f, names=rep(1:5, each=3), ylab="BODE", col=c("grey50", "purple", "yellow"), las=1, xlab="Sample cluster")
boxplot(fev1.fvc ~ status + sample.cluster.f, names=rep(1:5, each=3), ylab="FEV1/FVC", col=c("grey50", "purple", "yellow"), las=1, xlab="Sample cluster")

plot(1:10, type="n", xaxt='n', yaxt='n', ann=FALSE, axes=FALSE, bty="n")
legend("center", legend = c("Control", "ILD", "COPD"), fill = c("grey50", "purple", "yellow"))

dev.off()


## Creates meta-score for each mir cluster and plots a boxplot
pdf("Mir_Cluster_Meta_Scores.pdf", useDingbats=FALSE, width=8.5, height=4)
par(mfrow=c(2,2), mar=c(2,4,2,2))
mir.meta <- c()
box.col <- c("grey50", "red", "green", "blue")
for(i in 1:4) {
  # Run PCA
  e <- exprs(data.rpm)[sig.ind,]
  ix <- which(cc.mir.cluster == i)
  p <- prcomp(t(scale(t(e[ix,]))), center=FALSE, scale=FALSE)
  r <- p$rotation[,1]
  ...
  # Plot
  boxplot(r ~ sample.cluster.f, col=box.col[i], main = paste0("Module M", i), las=1, ylab="Meta-score")
  mir.meta <- rbind(mir.meta, r)
}
dev.off()

## Determine p-values between miR meta clusters and sample clusters
summary(lm(mir.meta[1,] ~ reads + protocol + smoke + gender + age + status + sample.cluster.f))
summary(lm(mir.meta[2,] ~ reads + protocol + smoke + gender + age + status + sample.cluster.f))
summary(lm(mir.meta[3,] ~ reads + protocol + smoke + gender + age + status + sample.cluster.f))
summary(lm(mir.meta[4,] ~ reads + protocol + smoke + gender + age + status + sample.cluster.f))
