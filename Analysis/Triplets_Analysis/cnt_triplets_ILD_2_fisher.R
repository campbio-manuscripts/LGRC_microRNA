...
write.table(mir_t_ord_desc, "Freq_miRNA_ILD_new_fisher1_corr.txt", sep="\t", row.names=TRUE, col.names=TRUE, quote=FALSE)

mir_t_ord <- order(-mir_t[,2])
mir_t_ord_desc <- mir_t[mir_t_ord,]
write.table(mir_t_ord_desc, "Freq_miRNA_ILD_new_fisher2_corr.txt", sep="\t", row.names=TRUE, col.names=TRUE, quote=FALSE)
