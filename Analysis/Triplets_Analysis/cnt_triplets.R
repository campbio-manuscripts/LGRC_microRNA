...
write.table(triplets2, "triplets_miRNA_gene_unique_Control.txt", sep="\t", row.names=TRUE, col.names=TRUE, quote=FALSE)

triplets2 <- read.table("triplets_miRNA_gene_unique_Control.txt", sep="\t", header=TRUE)
...
for (i in mirs_unique){
    ...
    for (j in 1:nrow(triplets_cit)){
     ...
     mir_t[i, "Freq"] <- nr/nrow(triplets_cit)#length(mirs_unique)    
    }
}
