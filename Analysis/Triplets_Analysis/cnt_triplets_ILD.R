...
write.table(triplets2, "triplets_all_unique_ILD.txt", sep="\t", row.names=TRUE, col.names=TRUE, quote=FALSE)

triplets_cit <- triplets2[which(triplets2$Causal_Call==1),]
nrow(triplets_cit)
triplets_cit_ord <- order(triplets_cit$P_Causal)
write.table(triplets_cit[triplets_cit_ord,], "triplets_cit_ord_unique_ILD.txt", sep="\t", row.names=TRUE, col.names=TRUE, quote=FALSE)
...
# }
# }
write.table(triplets3, "triplets_miRNA_gene_unique_ILD.txt", sep="\t", row.names=TRUE, col.names=TRUE, quote=FALSE)
...
write.table(mir_t_ord_desc, "Freq_miRNA_ILD.txt", sep="\t", row.names=TRUE, col.names=FALSE, quote=FALSE)
